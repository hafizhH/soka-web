      ...
      <div className='w-[65%] max-lg:w-full flex flex-col space-y-12 max-md:space-y-8'>
        <div className='flex flex-col'>
          ...
          <div className='mt-6 max-md:mt-4 flex flex-row space-x-3'>
            <img src={post?.fields.author.fields.profilePicture?.fields?.file?.url ?? '/blankprofile.png'} onError={({ currentTarget }) => { currentTarget.onerror = null; currentTarget.src = 'https://upload.wikimedia.org/wikipedia/commons/5/59/Empty.png?20091205084734' }} alt='profile' className='w-12 h-12 rounded-full bg-gray-300 border border-gray-400' />
            <div className='my-auto text-gray-500 max-md:text-sm'>{ post?.fields.author.fields.name ?? '' }</div>
          </div>
          {
            (post && post.fields.coverImage) ? <img src={post.fields.coverImage?.fields?.file?.url ?? '#'} alt={post.fields.coverImage?.fields?.title ?? 'Cover Image'} className='mt-6 w-full aspect-[5/3] object-cover bg-gray-400' /> : ''
          }
        </div>
              ...
              return (
                <a href={'/post/' + item.sys.id} key={index} className='cursor-pointer'>
                  <div key={index} className='py-2 flex flex-row space-x-4'>
                    ...
                    <img src={item.fields.coverImage?.fields?.file?.url ?? ''} onError={({ currentTarget }) => { currentTarget.onerror = null; currentTarget.src = 'https://upload.wikimedia.org/wikipedia/commons/5/59/Empty.png?20091205084734' }} alt={item.fields.coverImage?.fields?.title ?? 'Cover Image'} className='w-32 aspect-[4/3] object-cover bg-gray-300' />
                  </div>
                </a>
              )
            }) : ''
